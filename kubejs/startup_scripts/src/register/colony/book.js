const { RARE, EPIC, LEGENDARY, OVERLIMIT } = require("../../utils/itemborder")

StartupEvents.registry('item', event => {
    event.create('book_of_knowledge', 'basic')
        .maxStackSize(16)
        .texture('kubejs:item/book_of_knowledge')
    event.create('book_of_enlight', 'basic')
        .maxStackSize(16)
        .texture('kubejs:item/book_of_enlight')

    event.create('raid_book', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/raid_book')
    event.create('undead_raid_book', 'basic')
        .maxStackSize(16)
        .rarity('epic')
        .tag(EPIC)
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/undead_raid_book')

    event.create('skill_book_strength')
        .displayName(Text.translatable('item.kubejs.skill_book_strength').gold())
        .maxStackSize(16)
        .tag(LEGENDARY)
        .texture('kubejs:item/skill_book_strength')
    event.create('skill_book_agility')
        .displayName(Text.translatable('item.kubejs.skill_book_agility').gold())
        .maxStackSize(16)
        .tag(LEGENDARY)
        .texture('kubejs:item/skill_book_agility')
    event.create('skill_book_intelligence')
        .displayName(Text.translatable('item.kubejs.skill_book_intelligence').gold())
        .maxStackSize(16)
        .tag(LEGENDARY)
        .texture('kubejs:item/skill_book_intelligence')
    event.create('skill_forget_book')
        .displayName(Text.translatable('item.kubejs.skill_forget_book').gold())
        .maxStackSize(16)
        .tag(LEGENDARY)
        .texture('kubejs:item/skill_forget_book')

    event.create('master_certificate', "basic")
        .maxStackSize(16)
        .displayName(Text.translatable('item.kubejs.master_certificate').red())
        .tag(OVERLIMIT)
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/master_certificate')

    event.create('soul_gem_adaptability', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_adaptability')
    event.create('soul_gem_agility', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_agility')
    event.create('soul_gem_athletics', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_athletics')
    event.create('soul_gem_creativity', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_creativity')
    event.create('soul_gem_dexterity', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_dexterity')
    event.create('soul_gem_focus', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_focus')
    event.create('soul_gem_intelligence', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_intelligence')
    event.create('soul_gem_knowledge', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_knowledge')
    event.create('soul_gem_mana', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_mana')
    event.create('soul_gem_strength', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_strength')
    event.create('soul_gem_stamina', 'basic')
        .maxStackSize(16)
        .rarity('rare')
        .tag(RARE)
        .tag('kubejs:soul_gem')
        .tag('minecolonies:hide_citizen_data')
        .texture('kubejs:item/soul_gem_stamina')
})