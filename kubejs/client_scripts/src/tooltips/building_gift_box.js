ItemEvents.tooltip((tooltip) => {
    tooltip.addAdvanced('kubejs:building_gift_box', (item, advanced, text) => {
        ...
        if (item.nbt.contains('levelReq')) {
            text.add(lineNum++ , Text.translatable('tooltip.item.kubejs.building_gift_box.3', Text.gold(item.nbt.getString('levelReq'))).gray())
        }
        if (item.nbt.contains('force') && item.nbt.getInt('force') == 1) {
            text.add(lineNum++ , Text.translatable('tooltip.item.kubejs.building_gift_box.4').darkRed())
        }
